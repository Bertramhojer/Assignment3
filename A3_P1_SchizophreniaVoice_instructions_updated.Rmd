...
# extracting relevant data from filename and extracting numbers
...
# Now we merge them by their uniquely valid columns
...
data = data %>% mutate(uPairID = paste(subj, study, sep = "_"), # creating pairID from subj & study
                 uPairID = as.numeric(as.factor(uPairID)), # numeric to remove unncessary information
                 uID = paste(subj, study, diagnosis, sep = "_"), # creating ID from subj, study and diag
                 uID = as.numeric(as.factor(uID))) # numeric to remove unnecessary information
...
# Descriptive statistics diagnosis vs no-diagnosis
data %>% 
    filter(trial == 1) %>% 
    group_by(diagnosis) %>% 
    summarize(participants = n(),
              ...
              phonationTime = mean(phonationtime..s., na.rm = T),
              syllablesPrSec = mean(speechrate..nsyll.dur., na.rm = T),
              articulationRate = mean(articulation.rate..nsyll...phonationtime., na.rm = T)
    )
...
# changing any datapoints in the 'number of pauses' variable as these will produce data-points of value 'Inf (infinity)' as we're then dividing by 0. This will prove a problem for modeling. Therefore the data-points with value 0 are changed to NA.
data$npause[data$npause==0] <- NA

data = data %>% group_by(study) %>% mutate(pitchScaled = scale(mean), #mean of pitch
                                           # average syllables pr. sec
                                           speechrateScaled = scale(speechrate..nsyll.dur.),
                                           # duration of recording with speech present
                                           phonationtimeScaled = scale(phonationtime..s.),
                                           # syllables said pr. spoken second
                                           articulationrateScaled =    scale(articulation.rate..nsyll...phonationtime.),
                                           # The proportion of spoken time throughout the recording
                                           propSpeakingTimeScaled = scale((phonationtime..s./dur..s.)),
                                           # pause duration - (duration minus spoken time) / n() pauses
                                           pauseDurationScaled = scale((dur..s. - phonationtime..s.) / npause)
                                           )


# defining models
# Testing pitch variablity by diagnosis
modelPitch <- lmer(pitchScaled ~ diagnosis + (1 | uID), data)
summary(modelPitch)
cohen.d(data$pitchScaled, data$diagnosis, na.rm=T, pooled=T, hedges=T)

# Testing speech-rate by diagnosis
modelSpeechrate <- lmer(speechrateScaled ~ diagnosis + (1 | uID), data)
summary(modelSpeechrate)
cohen.d(data$speechrateScaled, data$diagnosis, na.rm=T, pooled=T, hedges=T)

# Testing proportion of spoken time by diagnosis
modelSpokentime <- lmer(propSpeakingTimeScaled ~ diagnosis + (1 | uID), data)
summary(modelSpokentime)
cohen.d(data$propSpeakingTimeScaled, data$diagnosis, na.rm=T, pooled=T, hedges=T)

# Testing pause-duration against diagnosis
modelPauseDuration <- lmer(pauseDuration ~ diagnosis + (1 | uID), data)
summary(modelPauseDuration)
cohen.d(data$pauseDurationScaled, data$diagnosis, na.rm=T, pooled=T, hedges=T)

# Writing a data-file to be accessed in part two
write_csv(data, "dataPart2.csv")
...
* In our case we decided to only use the danish studies to make the analysis less complex and to make it more comparable between studies. There are many confounds between running the studies in different countries and different cultural factors that might play in. 
Overall for all danish studies we can report 119 controls (male = 69, female = 49) and 104 diagnosed patients (male = 60, female = 44). As seen here the distribution of the genders is slightly skewed with more men participating in the studies.
The average age for control vs. patients is 26.6 and 26.8 respectively, we expect this since the patients were matched to controls.
We furtermore see that there's a huge difference in negative symptoms shown between groups (controls = 0.42, patients = 9.64), in positive symptoms (controls = 0.08, patients = 10.32), in IQ-score (controls = 103, patients = 87.5).


2) Describe the acoustic profile of a schizophrenic voice: which features are different?
E.g. People with schizophrenia tend to have high-pitched voice, and present bigger swings in their prosody than controls. N.B. look also at effect sizes. How do these findings relate to the meta-analytic findings?

* As to the autistic profile of patients vs. controls none of the hypothesis that we have seem to hold up. We have an expectation that the mean-pitch should be higher for patients, this however doesn't seem to be the case (mean pitch  = (controls = 160.48, patients = 151.66)). We furthermore had an expectation that patients would vary more in pitch, this also wasn't the case (controls SD = , 34.36 patients SD = 21.16). This doesn't go very well in line with the meta-analysis that found significant differences in pitch-variability and deviation in prosody in the opposite direction.
